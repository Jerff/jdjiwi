    ...
    public function html($param, $style = '') {
        ...
        if (is_null($param)) {
            ...
            foreach ($this->getValueArray() as $key) {
                ...
                $str .= " <label><input $style type=\"radio\" name=\"$name\" value=\"" . \Jdjiwi\Str::specialchars($key) . "\" {$select} id=\"{$name}_{$key}\" />"
        ...
        $input = "<input $style type=\"radio\" name=\"$name\" value=\"" . \Jdjiwi\Str::specialchars($param) . "\" {$select} {$option} id=\"{$name}_{$param}\" />";
