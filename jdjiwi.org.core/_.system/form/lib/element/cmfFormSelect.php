...
class cmfFormSelect extends cFormElement {
    ...
    protected function selectElementFind(&$values, &$sel, $find) {
        foreach ($values as $key => $value) {
            if (is_array($value)) {
                ...
                if (!is_array($sel[$key]) or ! sizeof($sel[$key]))
                    unset($sel[$key]);
            } else {
                if (is_array($find)) {
                    if (in_array($key, $find, false))
                        $sel[$key] = 1;
    ...
    protected function getValueFind(&$sel, &$values) {
        foreach ($sel as $key => $value) {
            if (is_array($value))
                $this->getValueFind($value, $values);
            else
                $values[] = $key;
        }
    }
    ...
    public function getValueArray() {
        ...
        $result = array();
        if (isset($values['array']))
            $this->getValueFind($values['array'], $result);
        return $result;
    }
            ...
            foreach ($values['array'] as $level1 => $value) {
                if (is_array($value)) {
                    ...
                    foreach ($value as $level2 => $value2) {
                        ...
                        $html .= "\n<option value=\"" . \Jdjiwi\Strings::specialchars($level2) . "\" $select $option>" . str_replace(' ', '&nbsp;', \Jdjiwi\Strings::specialchars($value2)) . "</option>";
                    }
                } else {
                    $select = ((isset($values['sel'][$level1])) ? 'selected' : '');
                    $option = $this->getOptionsStr($level1);
                    $html .= "\n<option value=\"" . \Jdjiwi\Strings::specialchars($level1) . "\" $select $option>" . str_replace(' ', '&nbsp;', \Jdjiwi\Strings::specialchars($value)) . "</option>";
                }
            }
        }
        return $html .= "\n</select>";
        ...
        if (isset($values['array']) and is_array($values['array'])) {
            foreach ($values['array'] as $k => $v) {
                ...
                $html .= "<span class=\"formCheckbox\"><label><input type=\"checkbox\" name=\"{$id}[{$k}]\" id=\"{$id}[{$k}]\" {$select} />"
                        . "&nbsp;" . \Jdjiwi\Strings::specialchars($v) . "&nbsp;</label>" . '</span>' . $sep;
            }
        }
        return $html;
    }
...
class cmfFormSelectCheckboxParam extends cmfFormSelectCheckbox {
    ...
    public function html($param, $sep = '') {
        ...
        if (isset($values['array']) and is_array($values['array'])) {
            foreach ($values['array'] as $k => $v) {
                $select = isset($values['sel'][$k]) ? 'checked' : '';
                $v = \Jdjiwi\Strings::specialchars($v);
...
HTML;
    ...
    public function htmlOld() {
        return parent::htmlOld() . '<input type="hidden" name="' . ($name = $this->getParamOldId()) . '" id="' . $name . '" value="' . \Jdjiwi\Strings::specialchars($this->dataParam) . '" />';
    }
    ...
    public function processing($data, $old, $upload) {
        ...
        if ($this->getOld() and $old) {
            ...
            if ($dataParam === $valueOld)
                $dataParam = null;
            else if ($dataParam == $valueOld)
                $dataParam = null;
        }
        ...
        $value = array();
        if (!is_null($res))
            $value[$this->getElementId()] = $res;
        if (!is_null($dataParam))
            $value[$this->indexParam] = $dataParam;
